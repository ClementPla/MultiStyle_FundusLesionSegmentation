...
def tjdr_testing_cssn(
    ...
    model = (
        CSNNStyleModel.from_pretrained("ClementP/MultiStyle_FundusLesionSegmentation", revision="CSNN").eval().cuda()
    )
    metrics = model.single_test_metrics
    print(model.classes_label)
    if on_train:
        dataloader = datamodule.train_dataloader()
    else:
        dataloader = datamodule.test_dataloader()
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        tjdr_testing_cssn(
            ...
            project_name="TJDR Testing CSSN - V3",
        )
        tjdr_testing_cssn(
            on_train=False,
            target_conversion=dataset,
